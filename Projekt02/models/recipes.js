...
export function getRecipe(idOrName) {
  ...
  let found = recipes.find(r => r.name === target);
  if (!found) {
    found = recipes.find(r => r.name.toLowerCase() === target.toLowerCase());
  }
  return found || null;
}

export function addRecipe(payload) {
  if (!payload || !payload.name || !payload.description) return null;
  ...
  const newRecipe = {
    ...
    instructions: payload.instructions ? String(payload.instructions) : ''
  };
